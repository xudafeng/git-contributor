...
exports.genMarkDown = (list, readMeContext = '') => {
  ...
  contentFirstLineData.map((item, key) => {
    contentFirstLine.push(`[<img src="${item.avatar_url}" width="80px;"/><br/><sub><b>${item.login}</b></sub>](${item.html_url})<br/>`);
  });
  contentFirstLine.push('');
  const contentRemaining = [];
  list.map((item, key) => {
    contentRemaining.push(`[<img src="${item.avatar_url}" width="80px;"/><br/><sub><b>${item.login}</b></sub>](${item.html_url})<br/>`);
  });

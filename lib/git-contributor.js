...
const getInfoFromGithubByUsernames = usernames => {
...
const parseOwnersFile = (ownersPath) => {
  ...
  return _.uniq(content.split(/\r?\n/).map(line => line.trim()).filter(Boolean).map(line => {
    const githubMatch = line.match(/github\.com\/(\w[\w-]*)/i);
    if (githubMatch) {
...
const mergeContributors = (primary, secondary) => {
  const map = new Map();
  primary.forEach(item => {
    if (item && item.login && !map.has(item.login)) {
      map.set(item.login, item);
    }
  });
...
exports.getAuthor = async (options = {}) => {
  ...
  if (ownersPath) {
    ...
    if (owners.length) {
      const ownerInfoList = await getInfoFromGithubByUsernames(owners);
      authorList = mergeContributors(format(ownerInfoList), authorList);
    }
  }
